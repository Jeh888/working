...
import {
  Building2, ChevronDown, ChevronRight, Moon, Sun,
...
export default function Header({ showSearch = false, searchPlaceholder = 'Search any NYC address...' }: HeaderProps) {
  ...
  useEffect(() => {
    const savedTheme = document.documentElement.getAttribute('data-theme') as 'dark' | 'light' | null
    if (savedTheme) {
      setTheme(savedTheme)
    }
  }, [])
  ...
  const serviceItems = [
    ...
    { href: '/services/tenant-lawyers', icon: Settings, color: 'blue', title: 'Tenant Lawyers', desc: 'Legal representation' },
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4">
          <div className="flex items-center justify-between gap-4">
            ...
            <Link href="/" className="flex items-center gap-2 sm:gap-3 group flex-shrink-0">
              <div className="relative w-9 h-9 sm:w-11 sm:h-11 bg-gradient-to-br from-blue-500 via-blue-600 to-indigo-600 rounded-xl flex items-center justify-center shadow-lg shadow-blue-500/30 group-hover:shadow-blue-500/50 transition-shadow">
            ...
            <nav className="hidden md:flex items-center gap-1">
              ...
              <div className="dropdown relative">
                ...
                <div className="dropdown-menu absolute top-full left-0 mt-2 w-64 bg-[var(--bg-card)] border border-[var(--border-primary)] rounded-xl shadow-2xl p-2">
              ...
              <div className="dropdown relative">
                ...
                <div className="dropdown-menu absolute top-full left-0 mt-2 w-56 bg-[var(--bg-card)] border border-[var(--border-primary)] rounded-xl shadow-2xl p-2">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <button
                onClick={toggleTheme}
                className="relative w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-[var(--bg-hover)] hover:bg-[var(--border-primary)] border border-[var(--border-primary)] flex items-center justify-center transition"
                aria-label="Toggle theme"
              >
                <Sun className={`w-4 h-4 sm:w-5 sm:h-5 text-amber-400 absolute transition-all duration-300 ${theme === 'light' ? 'opacity-100 rotate-0 scale-100' : 'opacity-0 rotate-90 scale-0'}`} />
                <Moon className={`w-4 h-4 sm:w-5 sm:h-5 text-blue-400 absolute transition-all duration-300 ${theme === 'dark' ? 'opacity-100 rotate-0 scale-100' : 'opacity-0 -rotate-90 scale-0'}`} />
              </button>
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 border-t border-[var(--border-primary)]">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={toggleTheme}
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[var(--bg-hover)] border border-[var(--border-primary)]"
              >
                {theme === 'dark' ? (
                  <>
